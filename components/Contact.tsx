...
export default function Contact({ settings }: { settings: SiteSettings | null }) {
  ...
  return (
    <section id="contact" ref={sectionRef} className="py-20 lg:py-32 bg-[#F7F5F2]">
      <div className="mx-auto max-w-[1400px] px-4 sm:px-6 lg:px-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16">
          ...
          >
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-1.5">
                  {hoursData.map((row) => (
                    <div key={row.day} className="contents text-[14px]">
                      <span className="text-[#1A1A1A] font-medium whitespace-nowrap">{row.day}</span>
                      <span className="text-[#6B7280] text-right whitespace-nowrap">{row.time}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-4">
              {phoneRevealed && (
                <a
                  href={`tel:${phone.replace(/\//g, '')}`}
                  className="inline-flex items-center justify-center gap-2 bg-[#0B1D2E] text-white text-[13px] font-medium px-6 py-3.5 rounded-full hover:bg-[#1A3A5C] transition-colors duration-200"
                >
              ...
              <a
                ...
                className="inline-flex items-center justify-center gap-2 border border-[#E5E7EB] text-[#0B1D2E] text-[13px] font-medium px-6 py-3.5 rounded-full hover:border-[#0B1D2E]/30 transition-colors duration-200"
              >
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          >
            <div className="relative rounded-2xl overflow-hidden bg-[#E5E7EB] h-[280px] sm:h-[350px] lg:h-full lg:min-h-[500px]">
              <iframe
                src={mapsEmbedUrl}
                width="100%"
                height="100%"
